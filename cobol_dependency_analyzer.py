...
DEPENDENCY_PATTERNS: list[tuple[str, re.Pattern]] = [
    ...
    #
    # Pattern structure: ^.{0,6}\s+COPY
    #   ^.{0,6}  — columns 1-6 (sequence / change-number area): spaces OR any developer
    #              prefix such as "CPK001".  Up to 6 characters, any content.
    #   \s+      — at least one whitespace before the verb (column 7 is the indicator
    #              column; code starts at column 8 — backtracking handles shorter prefixes).
    # Comment filtering is done post-match by _is_comment_line; no lookahead needed.
    (
        "COPY",
        re.compile(
            r"^.{0,6}"           # sequence area (cols 1-6: spaces or developer prefix)
            r"\s+"               # indicator column + indent — at least one space before verb
            r"COPY\s+"
    ...
    # Same ^.{0,6}\s+ structure as COPY for the same sequence-area reason.
    (
        "CALL",
        re.compile(
            r"^.{0,6}\s+"
    ...
    # EXEC SQL INCLUDE <member> END-EXEC  — DB2 DCLGEN copybook inclusion
    # Functionally identical to COPY: brings in SQL table declarations generated by DCLGEN.
    # The member name follows immediately after INCLUDE and maps to a .CPY file.
    # No ^.{0,6} anchor needed — EXEC can appear after indentation at any column.
    (
        "SQL INCLUDE",
        re.compile(
